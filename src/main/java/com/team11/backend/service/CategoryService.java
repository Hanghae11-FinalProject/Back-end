...
public class CategoryService {
    ...
    public List<CategoryResponseDto> categoryFilter(CategoryDto.RequestDto categoryRequestDto, Pageable pageable) {
        ...
        return posts.stream().map(s -> new CategoryResponseDto(s.getCategory(), s.getId(), s.getUser().getUsername(), s.getUser().getNickname(), s.getTitle(), s.getContent(), s.getUser().getAddress(), s.getMyItem(), s.getExchangeItem(),s.getImages(), s.getCurrentState(), TimeConversion.timeConversion(s.getCreateAt())))
