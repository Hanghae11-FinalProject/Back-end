...
public class BookMarkService {
    ...
    public List<BookMarkDto.ResponseDto> findMyBookMark(User user) {
        ...
        return bookMarkList.stream().map(s -> new BookMarkDto.ResponseDto(s.getPost().getId(), s.getPost().getTitle(),s.getPost().getImages().get(0).getImageUrl() ,s.getPost().getCurrentState().name())).collect(Collectors.toList());
    }
